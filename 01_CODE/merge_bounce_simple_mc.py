#!/usr/bin/env python3
"""
병합 모드 FSM 단일 파일 · DMABS (규칙4→SAFE, MA5>M120 시 안전→반등 100%).
방어(DEFENSE): 손절/트레일 후 **1.03×MA200 상향 돌파**(전일 이하·당일 초과) 또는 MA5>M120 상향이면 전액 반등 + 트레일 시드.
반등 종가만 트레일 · 공격 레짐 없음.

시그널·MA200은 항상 QQQ. 안전·방어·반등은 CLI (기본 safe=Gold, defense=QQQ, bounce=TQQQ).

  python3 01_CODE/merge_bounce_simple_mc.py \\
...
def _cross_up_103_ma(
    ...
    *,
    ...
    """전일: sig ≤ mult·전일MA200 이고 당일: sig > mult·당일MA200 (종가 기준 상향 돌파)."""
    mu = MULT_UP if mult_up is None else float(mult_up)
    if not (ma_ok and prev_ma_ok):
        return False
    if any(map(math.isnan, (sv, mah, prev_sv, prev_mah))):
        return False
    return prev_sv <= mu * prev_mah + 1e-12 and sv > mu * mah + 1e-12
    ...
    *,
    ...
    for k in range(n):
        ...
        if cross_dn:
            ...
            sig_break = _cross_up_103_ma(sv, mah, ma_ok, prev_sv, prev_mah, prev_ma_ok, mult_up=mu)
            if sig_break:
                if bv <= 0:
            ...
            if _cross_up_103_ma(sv, mah, ma_ok, prev_sv, prev_mah, prev_ma_ok, mult_up=mu):
                if bv <= 0:
        ...
        elif r == BOUNCE and _cross_up_103_ma(sv, mah, ma_ok, prev_sv, prev_mah, prev_ma_ok, mult_up=mu):
            if bv <= 0:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="병합 FSM + 단일 조합 MC Sortino")
    ...
    print(f"전구간 NAV · bounce={bt} defense={dt} safe={st} …")
    ...
    stem = (
        f"merge_bounce_simple_mc_{fx0}_{fx1}_{tk_b}_{tk_d}_{tk_s}_"
        ...
        json.dumps(
            {
                "meta": {
                    "strategy": (
                        ...
                        "(DMABS MA5>M120; DEF→Bounce 동조건; SAFE/BOUNCE 103MA는 상향돌파만)"
                    ),
                    "sig": SIG_TICKER,
                    "bounce": tk_b,
                    "defense": tk_d,
